...
- After landing on the employer page, fills application fields. Greenhouse/Lever/Ashby use split `first_name`/`last_name` fields — `cfg.user.name` is split on whitespace and each part fills its matching field; falls back to a single "full name" input if no split fields exist. Email, phone, LinkedIn URL, and resume file upload all run through CSS-selector heuristics (`autocomplete`, `name`, `id`, `placeholder`, `aria-label`, `type`).
- Apply-button click-through tries selectors most-specific first (`Apply on Company`, `Apply on company site`, `Apply Externally`, `Apply for this job`, then bare `Apply Now`/`Apply`). If a click doesn't navigate (e.g. The Muse's sticky "Apply Now" only scrolls), the next selector is tried instead of giving up.
- Selectors are appended with `:visible` so `.first` returns the first visible match rather than a hidden one. Critical for The Muse, which renders multiple copies of "Apply on company site" (header, sidebar, footer) where only the in-viewport copy is interactable.
- New-tab detection uses `page.context.on("page", ...)` attached **before** the click. Previously used `expect_page` after the click — that races and silently misses target=_blank pops.
